        ...
        implements ColumnWriter, DictionaryColumn
{
            ...
            MetadataWriter metadataWriter)
    {
        ...
        rowGroupIndexes = new int[10_000];
    }
    ...
    protected abstract boolean tryConvertRowGroupToDirect(int dictionaryIndexCount, int[] dictionaryIndexes, int maxDirectBytes, Optional<ColumnStatistics> statistics);
    ...
    public OptionalInt tryConvertToDirect(int maxDirectBytes)
    {
        ...
        for (DictionaryRowGroup rowGroup : rowGroups) {
            ...
            boolean success = tryConvertRowGroupToDirect(rowGroup.getValueCount(), rowGroup.getDictionaryIndexes(), maxDirectBytes, Optional.of(rowGroup.getColumnStatistics()));
        ...
        if (inRowGroup) {
            if (!tryConvertRowGroupToDirect(rowGroupValueCount, rowGroupIndexes, maxDirectBytes, Optional.empty())) {
                directWriter.close();
                directWriter.reset();
                return OptionalInt.empty();
            }
