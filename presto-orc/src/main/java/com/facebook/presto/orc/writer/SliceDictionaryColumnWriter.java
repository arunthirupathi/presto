        ...
        extends DictionaryColumnWriter
{
    ...
    @Override
    protected boolean tryConvertRowGroupToDirect(int dictionaryIndexCount, int[] dictionaryIndexes, int maxDirectBytes, Optional<ColumnStatistics> statistics)
    {
        ...
        for (int offset = 0; offset < dictionaryIndexCount; offset++) {
            ...
            if (dictionaryIndex != NULL_INDEX) {
                ...
                if (statistics.isPresent()) {
                    ...
                    directColumnWriter.writeSliceWithoutStatistics(rawSlice, rawSliceOffset, length);
                }
                else {
                    Slice slice = dictionary.getSlice(dictionaryIndex, length);
        ...
        statistics.ifPresent(columnStatistics -> directColumnWriter.finishRowGroup(columnStatistics));
    ...
    private static int[] getSortedDictionaryNullsLast(DictionaryBuilder dictionary)
    {
    ...
    protected Optional<int[]> writeDictionary()
    {
        ...
        int[] sortedDictionaryIndexes = getSortedDictionaryNullsLast(dictionary);
